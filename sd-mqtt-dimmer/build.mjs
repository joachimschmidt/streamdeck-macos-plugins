...
const PLUGIN_DIR = "com.local.mqtt-dimmer.sdPlugin";
...
await build({
  ...
  // Node built-ins are external (handled by platform: "node")
});
...
cpSync("layouts/dimmer.json", `${PLUGIN_DIR}/layouts/dimmer.json`);

// Create minimal placeholder icon (20x20 gray PNG)
// This is a valid minimal PNG so Stream Deck doesn't complain
